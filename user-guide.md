...
7. Flash the firmware to your _ctag-tbd_ and enjoy your newly acquired memory for samples. Make sure to flash the firmware via USB using [ESP Tool](https://github.com/espressif/esptool). Your download should contain a flash script you can run from the downloaded directory with `sh ./flash.sh`. This is necessary because bootloader and the overall memory partition also need to be updated which is not possible via the web ui. 
